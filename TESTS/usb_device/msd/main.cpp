...
FATFileSystem flash_fs("flash_fs");
...
/** Creates flah block device
 *
 * Limit its size to 1MB to speed up mounting time
 */
BlockDevice *get_flash_block_device()
{
    BlockDevice *default_bd = BlockDevice::get_default_instance();
    default_bd->init();
    if (default_bd->size() <= (1024 * 1024)) {
        return default_bd;
    }
    // Create a partition with 1 MB of space
    MBRBlockDevice::partition(default_bd, 1, 0x83, 0, 1024 * 1024);
    // Create the block device that represents the partition
    static MBRBlockDevice part1(default_bd, 1);
    return &part1;
}
...
void storage_init()
{
    ...
    if (bd) {
        ...
        result = prepare_storage(bd, &flash_fs);
        TEST_ASSERT_MESSAGE(result, "Flash block device init failed");
    } else {
...
void flash_block_device_mount_unmount_test()
{
    BlockDevice *bd = get_flash_block_device();
...
Case cases[] = {
    ...
    Case("mount/unmount test - Flash block device", flash_block_device_mount_unmount_test),
    Case("mount/unmount and data test - Flash block device", flash_block_device_mount_unmount_and_data_test)
};
